...
Commands: see `min-zettel --help`, `zk --help`, or README.
"""
...
HEADER_KEYS = ("id", "title", "created", "updated", "aliases", "tags")
...
class NoteIndex:
    def __init__(self, notes: List[NoteMeta]):
        ...
        self.by_tag: Dict[str, List[NoteMeta]] = {}
        for n in notes:
            for a in n.aliases:
                key = a.strip().lower()
                if key:
            ...
            for t in getattr(n, "tags", []):
                key = t.strip().lower()
                if key:
                    self.by_tag.setdefault(key, []).append(n)
...
def render_header(meta: Dict[str, str]) -> str:
    ...
    for k in HEADER_KEYS:
        # Always include 'tags' even if empty; others skip if empty
        if k in meta and (meta[k] != "" or k == "tags"):
...
def load_meta_from_path(cfg: Config, path: Path) -> NoteMeta:
    ...
    return NoteMeta(id=nid, title=title, created=created, updated=updated,
                    aliases=aliases, path=path, slug=slug, mtime=mtime, tags=tags)
...
def cmd_path(cfg: Config, args: argparse.Namespace) -> int:
    """
    Print the relative path './ID_slug.md' for a note selected by ID/alias/slug fragment.
    Exits non-zero on no match or ambiguous match (delegated to resolve_note).
    """
    note = resolve_note(cfg, args.selector)
    # print a stable, renderer-friendly relative path
...
def cmd_link(cfg: Config, args: argparse.Namespace) -> int:
    """
    Print a link string for the selected note.
      --style md   -> [Title](./ID_slug.md)      (default)
      --style wiki -> [[ID Title]]
    """
    note = resolve_note(cfg, args.selector)
    # Prefer header title; fall back to slug prettified
    title = note.title.strip() if getattr(note, "title", "").strip() else note.slug.replace("_", " ").title()
    if args.style == "wiki":
        print(f"[[{note.id} {title}]]")
    else:
        print(f"[{title}](./{note.path.name})")
    return 0
...
def cmd_find(cfg: Config, args: argparse.Namespace) -> int:
    ...
    res = []
    for n in idx.notes:
        header = f"{n.id} {n.title} {';'.join(n.aliases)} {n.created} {n.updated} {';'.join(getattr(n, 'tags', []))}"
...
def cmd_tag(cfg: Config, args: argparse.Namespace) -> int:
    idx = scan_notes(cfg)
    note = idx.by_id.get(args.id)
    if not note:
        print("Unknown ID", file=sys.stderr)
    ...
    text = read_text(note.path)
    headers, body_idx = parse_header(text)
    cur = [t.strip() for t in headers.get("tags", "").split(";") if t.strip()]
    new = [t.strip() for t in args.names.split(";") if t.strip()]
    have = {t.lower() for t in cur}
    for t in new:
        if t.lower() not in have:
            cur.append(t)
            have.add(t.lower())
    headers["tags"] = ";".join(cur)
    headers["updated"] = now_iso(cfg)
    body = "\n".join(text.splitlines()[body_idx:])
    write_text(note.path, render_header(headers) + body, bak=True)
    return 0

def cmd_taglist(cfg: Config, args: argparse.Namespace) -> int:
    idx = scan_notes(cfg)
    counts: Dict[str, int] = {}
    for n in idx.notes:
        for t in getattr(n, "tags", []):
...
def cmd_rename_tag(cfg: Config, args: argparse.Namespace) -> int:
    idx = scan_notes(cfg)
        ...
        for t in tags:
            if t.lower() == old:
                ...
                if t.lower() not in seen:
                    ...
                    seen.add(t.lower())
        if modified:
            headers["tags"] = ";".join(new_tags)
            headers["updated"] = now_iso(cfg)
            body = "\n".join(text.splitlines()[body_idx:])
            write_text(n.path, render_header(headers) + body, bak=True)
            changed += 1
    print(f"Renamed tag '{args.old}' -> '{args.new}' in {changed} notes.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Ultra-minimal CLI Zettelkasten")
...
# --- NEW: path ---
    sp = sub.add_parser("path", help="Print relative path './ID_slug.md' for a note")
    sp.add_argument("selector", help="ID, alias, or slug fragment")
    sp.set_defaults(func=cmd_path)

# --- NEW: link ---
    sp = sub.add_parser("link", help="Print a Markdown or wiki link for a note")
    sp.add_argument("selector", help="ID, alias, or slug fragment")
    sp.add_argument("--style", choices=["md", "wiki"], default="md",
                    help="Link style: md -> [Title](./ID_slug.md), wiki -> [[ID Title]]")
    ...
    sp = sub.add_parser("tag", help="Append tag(s) to header (semicolon-separated)")
    sp.add_argument("id")
    sp.add_argument("names", help="e.g. 'entropy; info-theory'")
    sp.set_defaults(func=cmd_tag)

    sp = sub.add_parser("taglist", help="List all tags with usage counts")
    sp.set_defaults(func=cmd_taglist)

    sp = sub.add_parser("rename-tag", help="Rename a tag across all notes")
    sp.add_argument("old")
    sp.add_argument("new")
    sp.set_defaults(func=cmd_rename_tag)
